      ...
      .then((data) => {
        ...
        data.forEach((el) => {
          ...
          movieTitle.addEventListener("click", function (e) {
            ...
            window.location.href = "./bitshow2.html";
            e.stopPropagation();
          });
        });
      });
...
function getSeasons(id) {
  fetch(`https://api.tvmaze.com/shows/${id}/seasons`)
    .then((res) => res.json())
    .then((data) => {
      let seasonsNum = document.createElement("ul");
...
function getCast(id) {
  fetch(`https://api.tvmaze.com/shows/${id}/cast`)
    .then((res) => res.json())
    .then((data) => {
      let castTitle = document.createElement("ul");
      castTitle.innerText = "Cast";
      cast.append(castTitle);
      let newData = data.filter((element, index) => index < 15);
